...
| Captions | Under each node's label and sublabel, the node `description` is wrapped to at most 2 lines at 12px and ends with "…" if longer. Nodes without a description get no caption. |
| Edge text | An edge `description` or `condition` is drawn as a small label on its connection only where it fits clear of nodes, labels and other text. It is never forced in. |
