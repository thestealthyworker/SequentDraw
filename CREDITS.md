...
| [Simple Icons](https://github.com/simple-icons/simple-icons) | Brand icon paths and colours, looked up by slug in `src/icons.js` and `src/scan/crosswalk.js`, and named by slug in the integration catalogue (`src/catalogue/integrations.js`); every catalogue slug is checked against the installed package by `tests/catalogue.test.js`. | CC0-1.0 |
| [@specfy/stack-analyser](https://github.com/specfy/stack-analyser) 1.27.6 — **vendored, no longer a dependency** | Its eight dependency-manifest parsers (Go, Rust, Ruby, PHP, Deno, Terraform lockfile and resources, GitHub Actions `uses:`) were ported into `src/scan/rules/dependency-manifests.js` in build step 8a, and its 166 Terraform resource patterns extracted as data into `src/scan/rules/terraform-resources.js`; the package and its 39-package transitive tree were then removed. Its MIT licence is kept at `src/scan/rules/LICENSE-stack-analyser`. Licence re-verified on 2026-09-25 via `gh api repos/specfy/stack-analyser/license` (`MIT`). | MIT |
| [yaml](https://github.com/eemeli/yaml), pinned to exactly `2.9.1` | Bounded YAML parsing for docker-compose and GitHub Actions workflow files (`src/scan/yaml-safe.js`), with `maxAliasCount`, a nesting-depth cap, and an input-size cap so a crafted "alias bomb" or deeply-nested document cannot exhaust memory, overflow the stack, or hang the scan. `2.9.1` is patched against the published stack-overflow advisory on deeply nested YAML, and since step 8a it is the only copy installed (stack-analyser's nested `2.8.x` went with it, and so did the `overrides` that pinned it). `src/scan/safe-provider.js` still runs every `*.yml`/`*.yaml` file it opens through these bounded settings before any parser sees it. | ISC |
| [@iarna/toml](https://github.com/iarna/iarna-toml), pinned to exactly `2.2.5` | Parses `Cargo.toml` for the Rust dependency parser vendored in step 8a (`src/scan/rules/dependency-manifests.js`). It was already installed as one of stack-analyser's dependencies and is now a direct one, so Rust detection reads Cargo files exactly as before. No dependencies of its own. Licence verified on 2026-09-25 via `gh api repos/iarna/iarna-toml/license` (`ISC`). | ISC |
...
The integration catalogue (`src/catalogue/`) lists products that n8n offers
integrations for. It is written by SequentDraw: each entry holds a product name
(a public fact), SequentDraw's own id, category and one-line description, and a
Simple Icons slug or none. No n8n source, node definitions, descriptions, node
type ids, icons, credential schemas or popularity data are included. Each name
was checked on 2026-09-17 by confirming that its public page under
`https://n8n.io/integrations/` exists and is titled with that name; only the
name was read.
