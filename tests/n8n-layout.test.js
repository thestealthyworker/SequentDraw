// Invariant tests for the n8n-style renderer, run against the Medusa fixture
// for both layout strategies. See docs/design/n8n-visual-style.md "Testing".
...
for (const strategy of ['flat', 'rows']) {
  describe(`strategy "${strategy}" on the Medusa fixture`, () => {
    let layout;
    let html;

    test('layoutMap resolves without throwing', async () => {
      layout = await layoutMap(doc, { strategy });
      assert.ok(layout);
    });
