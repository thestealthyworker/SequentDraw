...
// via its own MCP config) reach the engine through this process; Claude
// Code gets it bundled through .mcp.json. Every tool call runs the exact
...
function runServer(io = {}) {
  ...
  // never appears inside a multi-byte sequence). `discarding` is true while
  // skipping the remainder of a line already rejected as oversized, so an
  // arbitrarily long line without any newline yet still costs bounded
  // memory rather than growing forever while we wait for one.
  ...
  function drainBuffer() {
    for (;;) {
      if (discarding) {
        const idx = buffered.indexOf(NEWLINE);
        if (idx === -1) {
      ...
      const lineBuf = buffered.subarray(0, idx);
      buffered = buffered.subarray(idx + 1);
      handleLine(lineBuf);
    }
  }
  ...
  return new Promise(resolve => {
    stdin.on('data', chunk => {
      ...
      buffered = Buffer.concat([buffered, asBuffer]);
      drainBuffer();
    });
